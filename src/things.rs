...
static mut past : Vec<(i32, i32, i32)> = vec![];
...
) -> Vec<(i32, i32, i32)>
{
    ...
    for (x, z) in itertools::iproduct!(range.clone(),range.clone()){
        ...
        //     continue;
        // }
        let yi = (amp * cos((x as f64/pis).into())) + (amp *cos((z as f64/pis).into()));
        // println!("{:?}", yi);
        let mut y = (SPAWN_POS.y - yi).round().abs();
        y = if y > 256.0 {yi} else {y};

        visual[(x + r) as usize][(z+r) as usize] = shadeness(y); 
        poses.push((x,y as i32,z));
    }
    for row in &visual {
        for &y in row {
            print!("{}", y);
    ...
    // println!("{:?}", (amp * cos((-49 as f64/pis).into())) + (amp *cos((49 as f64/pis).into())));
    return poses;
...
pub fn animate (
    ...
    unsafe {
        for (x, y, z) in past.clone() {
            layer
                .set_block([x, y , z], BlockState::AIR);
        }
        past = poses.clone();
    }
    for (x, y, z) in poses {
        layer
            .set_block([x, y , z], BlockState::RED_WOOL);
    }
